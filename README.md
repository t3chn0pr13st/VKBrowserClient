...
- 📼 Длинные VOD — сохраняемые этапы `video.save` → CDN upload → обработка → подтверждённый `privacy_view=by_link`, без обязательной записи на стене. Во время обработки отсутствие поля приватности считается промежуточным состоянием; готовая запись без readback-подтверждения не публикуется.
