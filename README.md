...
- 📼 Длинные VOD — сохраняемые этапы `video.save` → CDN upload → обработка → подтверждённый `privacy_view=by_link`, без обязательной записи на стене. Подтверждением служит либо readback поля приватности, либо возвращённый `video.edit` ключ доступа по ссылке; готовая запись без одного из этих сигналов не публикуется.
